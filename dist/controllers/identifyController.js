...
const identifyContact = async (req, res) => {
    try {
        ...
        const oldestPrimary = primaries.reduce((oldest, current) => current.createdAt < oldest.createdAt ? current : oldest);
        ...
        // ===============================
        const emailSet = new Set(finalCluster.map((c) => c.email).filter(Boolean));
        const phoneSet = new Set(finalCluster.map((c) => c.phoneNumber).filter(Boolean));
        ...
        const primaryContact = finalContacts.find((c) => c.linkPrecedence === "primary");
